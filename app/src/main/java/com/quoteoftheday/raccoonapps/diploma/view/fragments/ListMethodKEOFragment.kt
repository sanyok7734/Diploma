...
class ListMethodKEOFragment : Fragment() {
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        label.text = activity.resources.getString(R.string.method) + ":"

        sortTwo.setImageDrawable(activity.resources.getDrawable(R.drawable.ic_about_vector))

        recyclerView.layoutManager = LinearLayoutManager(activity)

        method = CacheDataRetriever.getFromFileListUsers(activity, Constants.METHOD_KEO, "MethodKEO")

        adapterListMethod = AdapterListMethod(method, activity,0)
    ...
    override fun onPause() {
        super.onPause()
        CacheDataRetriever.saveToFileListUsers(activity, method, Constants.METHOD_KEO)
    }
